...
function displayselected_blog($param1, $param2, $param3, $param4, $param5, $param6,$param7){
  if(isset($_SESSION['u_id'])){
  echo'<a href="userprofile.php?username='.$param2.'">
  <div class="post">
...
function displayContent_blogs($param1, $param2, $param3, $param4, $param5, $param6,$param7){
  ...
  function query_all($userName,$blogId)
        {
          include 'connection.php';
            if ($conn->connect_error) {
                die("Connection failed:" . $conn->connect_error);
            }
            $sql = "SELECT * FROM blogPost WHERE userName='".$userName."' AND blogId!=".$blogId." ORDER BY blogDate DESC;";
            $result = mysqli_query($conn, $sql);
            return $result;
        }
        function query_selected($param7)
              {
                include 'connection.php';
                  if ($conn->connect_error) {
                      die("Connection failed:" . $conn->connect_error);
                  }
                  $sql = "SELECT * FROM blogPost WHERE blogId='".$param7."';";
                  $result = mysqli_query($conn, $sql);
                  return $result;
              }
...
<body>
  ...
  <div id="main">
    <div id="main_column">
  <?php
  $resultselected = query_selected($blogId);
   $resultCheckselected = mysqli_num_rows($resultselected);
   if ($resultCheckselected > 0) {
       while ($row = mysqli_fetch_assoc($resultselected)) {
        displayselected_blog($row['blogTitle'], $row['userName'], date('Y-m-d', strtotime($row['blogDate'])), date('h:i a' , strtotime($row['blogDate'])), $row['blogContent'], $row['blogPic'],$row['blogId']);
      }
    }
    echo "<h1 style ='border-bottom: 1px solid black;'>Other Posts made by user</h1>";
    $result = query_all($userName,$blogId);
